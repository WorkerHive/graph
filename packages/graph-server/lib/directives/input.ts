...
export function transform(composer: SchemaComposer<any>, typeRegistry: TypeRegistry) : GraphQLSchema {
    ...
    let outputTypes = types.map(inputType => {
        ...
        inputFields = inputFields.filter(field => otc.getFieldDirectiveNames(field.name).filter((directive) => directive.indexOf(directiveName) > -1).length > 0);
        
        let inputFieldObj : any = {};
        
        generatedFields.forEach((field) => {
            inputFieldObj[field.name] = {
                type: field.type,
                extensions: {
                    directives: field.directives.map((x : any) => ({
                            name: x.name.value,
                            args: x.args || {}
                        }))
                }
