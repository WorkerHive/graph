...
import { monikerMiddleware } from './moniker'
...
export const applyGenerators = (resolver: GraphQLResolver, generators: Record<string, GraphGeneratorAction>) => {
    ...
    return async (parent : any, args : any = {}, context : GraphContext, info: GraphQLResolveInfo) => {
        schemaComposer.merge(info.schema)

        //Get input arguments 
        let inputArgs = info.parentType.getFields()[info.fieldName].args

        let generated : any = args || {}
...
let createGenerators : any = {};
let updateGenerators : any = {};
